		...
		while ((line = input.readLine()) !== null) {
			...
			if (line.length() === 0) { // double line-feed (request is done)
				...
				if (WebSocket.isWebSocketRequest(request)) {
					new WebSocket(request, client, input, output).loop(function (message) {
	...
	(function () {
		...
		apps.forEach(function (app) {
			...
			if (typeof app.hosts === 'undefined' || !app.hosts.length || typeof app.application === 'undefined') {
				throw new Error('There is a missing hosts or application property in http.applications.json');
			}

			load(app.application);

			if (applications.length === length) {
				throw new Error(app.application + ' did not call HTTPSserver.addApplication');
			}
			application = applications[applications.length - 1];

			/* Wrap a single string host into an array */
			if (typeof app.hosts === 'string') {
				app.hosts = [app.hosts];
			}

			/* Determine absolute file path for the application */
			var path = '' + app.application;
			matches = path.match(/([^\/]*)\//);
			path = new File(matches === null && path || matches[1] + '/');
