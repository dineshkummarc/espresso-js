...
(function (args) {
			...
			if (line.length() === 0) { // double line-feed (request is done)
				...
				if (!regex['request'].test(line)) { // This is an HTTP header
					...
					if (matches && matches.length === 4) {
						...
						request.resource = matches[2].toLowerCase();
	...
	HTTPServer.serveFile = function (request, file, cache) {
		var ext = ('' + file.getName()).match(regex['ext']), len = file.length(), data, buffer, cacheTime, expires;
		...
		request.output.print('HTTP/' + request.httpVersion + ' 200 OK\r\n');
		request.output.print('Content-Type: ' + mimes[ext] + '\r\n');
		request.output.print('Content-Length: ' + len + '\r\n');
		request.output.print('Expires: ' + expires + '\r\n');

		if (cacheTime) {
			request.output.print('Cache-Control: private, max-age=' + cacheTime + ', must-revalidate\r\n');

		} else {
			request.output.print('Cache-Control: no-cache, max-age=0, must-revalidate\r\n');
		}
