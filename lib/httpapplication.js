...
HTTPApplication.prototype.bufferView = function (name, output, context) {
	var out = new HTTPOutputStream(output);
	out.beginBuffer();
	this.views[name](out, context || {});
	return out.endBuffer();
};

/* Renders view directly to the output stream */
HTTPApplication.prototype.renderView = function (name, output, context) {
	return this.views[name](new HTTPOutputStream(output), context || {});
};
