...
(function () {
	...
	HTTPViewParser.addScope = function (code) {
		...
		instructions.unshift('with (new HTTPViewScope(output, request, context, headers, application || this)) {');
		instructions.unshift('function (output, request, context, headers, application) {');
		instructions.push('}}');
	...
	HTTPViewParser.parse = function (view) {
		return eval(HTTPViewParser.addScope(convert(view)));
	};
}());
