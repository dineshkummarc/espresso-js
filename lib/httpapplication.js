...
HTTPApplication.prototype.serveFile = function (request, file, cache, headers) {
	var ext = String(file.getName()).match(HTTPServer.regex.ext), len = file.length(), data, buffer, cacheTime, expires, headers = headers || {};
	if (!cache && +cache !== 0) {
	...
	headers['content-type'] = headers['content-type'] || HTTPServer.mimes[ext];
...
HTTPApplication.prototype.redirect = function (request, url, permanent) {
	this.sendResponseHeaders((permanent && 301) || 302, { 'Location': url }, request);
};

HTTPApplication.prototype.requireAuth = function (request, realm) {
	this.sendResponseHeaders(401, {
		'WWW-Authenticate': 'Basic realm="' + (realm || 'Secure area') + '"'
	}, request, 0);
};
