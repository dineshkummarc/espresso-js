...
HTTPApplication.prototype.serveFile = function (request, file, cache) {
	var ext = ('' + file.getName()).match(HTTPServer.regex['ext']), len = file.length(), data, buffer, cacheTime, expires, headers = {};
...
HTTPApplication.prototype.sendResponseHeaders = function (httpCode, headers, request, length) {
	...
	if (request.gzip) {
		request.output = new PrintStream(new GZIPOutputStream(request.getSocket().getOutputStream()));
	}
};
