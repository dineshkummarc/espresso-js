...
(function () {
	...
	function convert(input) {
		...
		function flushOutput() {
			...
			if (output.length) {
				instructions.push("this.output.print('" + output + "');");
				outputBuffer = '';
			}
		}
		...
		instructions.unshift('with (new HTTPViewScope(output, headers)) {');
		instructions.unshift('function (output, context, headers) {');
		instructions.push('}}');
