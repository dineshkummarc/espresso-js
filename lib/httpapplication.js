...
HTTPApplication.prototype.renderView = function (name, output, context, httpCode, headers) {
	...
	if (Object.keys(headers).filter(function (header) { return /location/i.test(header); }).length) {
		// Automatically set HTTP code to 302 if a Location header is being sent
		httpCode = 302;
	} else {
		httpCode = httpCode || 200;
	}

	this.sendResponseHeaders(httpCode, headers, output, data.length);
	output.print(data);
};
